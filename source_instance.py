#!/usr/bin/env python3
"""
Generate silver models for each netsuite source instance by reading dbt_project.yml
and copying/replacing tokens in reusable silver templates.

Features:
- Robust parsing of vars.netsuite.source_instances supporting multiple YAML shapes
  (list of strings, dict of instances, and list of ad-hoc dicts like in user's example).
- Creates per-instance silver models with unique filenames (e.g., account__netsuite1.sql)
...
Usage:
    python generate_silver_models.py \
        ...
        --templates-dir netsuite/silver \
        --output-dir models/netsuite/silver \
...
def _normalize_instances(si: Any) -> List[Dict[str, Any]]:
    """
    Normalizes vars.netsuite.source_instances into a list like:
        [{"name": "netsuite1", ...}, {"name": "netsuite2", ...}]
    Supports:
      1) list of strings: ['netsuite1', 'netsuite2']
      2) dict: {'netsuite1': {...}, 'netsuite2': {...}}
      3) list of single-key dicts whose value is dict OR list-of-dicts:
         - netsuite1:
           - source_database_name: netsuite_db
           - source_schema_name: netsuite1
           - tool_name: fivetran
    """
    ...
    if isinstance(si, list):
        for item in si:
            if isinstance(item, str):
                ...
                if "name" in item:
                    ...
                    # shape: {netsuite1: <dict or list of dicts>}
                    k = next(iter(item))
                    v = item[k]
                    inst = {"name": str(k)}
        ...
        raise ValueError("vars.netsuite.source_instances must be a list or a dict.")
...
def extract_netsuite_config(project: Dict[str, Any]) -> Dict[str, Any]:
    vars_dict = project.get("vars", {}) or {}
    netsuite = vars_dict.get("netsuite", {}) or {}

    defaults = {
        "source_database_name": netsuite.get("source_database_name") or vars_dict.get("source_database_name"),
        "source_schema_prefix": netsuite.get("source_schema_prefix") or vars_dict.get("source_schema_prefix"),
    }

    # Look for source_instances in netsuite first, then in vars root
    si = netsuite.get("source_instances") or vars_dict.get("source_instances")
...
def substitute_instance_token(text: str, instance_name: str) -> str:
    ...
    replaced = re.sub(r"netsuite__\s*__INSTANCE__", f"netsuite__{instance_name}", replaced)
    return replaced
...
WITH raw AS (
    SELECT
        *,
        {{ source_metadata() }}
    FROM {{ source('netsuite""" + instance_name + """', '""" + table_name + """') }}
    WHERE 1=1
    {{ incremental_filter() }}
),
...
def main():
    ...
    parser.add_argument(
        "--templates-dir",
        type=str,
        default="models/netsuite/silver",
    ...
    cfg = extract_netsuite_config(project)

    instances = cfg["source_instances"]
    if not instances:
        raise RuntimeError("No source instances found in dbt_project.yml under vars.netsuite.source_instances.")
